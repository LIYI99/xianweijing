...
// mouse image load
static void     _image_mouse_image_load(window_node_mouse_t *mouse,char *iamge,char *image2);
...
static void     _image_mouse_image_load(window_node_mouse_t *mouse,char *image,char *image2)
{

    mouse->save_cache = (char *)buddymem_alloc(sdk_handle->mems, mouse->size *mouse->size_w * sdk_handle->color_fmt / 8 *2);
    if(mouse->save_cache == NULL){
         xw_logsrv_err("the sdk mempool not ougeht\n");
    }
   
    mouse->save_cache_v2 = (char *)buddymem_alloc(sdk_handle->mems, mouse->size_v2 *mouse->sizew_v2 * sdk_handle->color_fmt / 8 *2);
    if(mouse->save_cache == NULL){
         xw_logsrv_err("the sdk mempool not ougeht\n");
    }
    return ;
...
static void     _image_mouse_image_deload(window_node_mouse_t *mouse)
{
    ...
    buddymem_free(sdk_handle->mems, mouse->save_cache_v2);
    return ;
...
//void    Image_SDK_Init(void){
    ...
    //base param init
    sdk_handle->en = 1;
    sdk_handle->scree_w     =   param->srcee_w;       //VO_SCREE_W;
    sdk_handle->scree_h     =   param->srcee_h;       //VO_SCREE_H;
    sdk_handle->base_scree_w =  param->base_srcee_w;
    sdk_handle->base_scree_h =  param->base_srcee_h;
    sdk_handle->color_fmt   =   param->color_fmt;   //VO_COLOR_FMT;
    sdk_handle->disp_fps    =   VO_DISP_FPS;    
    sdk_handle->font_h      =   param->font_h;
    sdk_handle->font_w      =   param->font_w;
    ...
    if(sdk_handle->video_fd < 0)
        goto ERR1;
    //open mouse fd 
    sdk_handle->mouse_fd =  gk_mouse_open(GK_DEVICE_MOUSE,param->srcee_w,
            param->srcee_h,1,param->mouse_h);
...
#define     SDK_MEM_BLOCK_NUMS      1024
    ...
    mouse->size         =  param->mouse_h;  //MOUSE_SIZE;
    mouse->size_w       =  param->mouse_w;
    mouse->size_v2      =  param->mouse_h_v2;
    mouse->sizew_v2     =  param->mouse_w_v2;
    mouse->mouse_buf    = param->mouse_cahce;
    mouse->mouseh_buf   = param->mouse_cahce_v2;
    
    _image_mouse_image_load(mouse,param->mouse_path,param->mouse_pathv2);
...
int    Image_SDK_Create_Button( struct user_set_node_atrr attr,
    ...
    //xw_logsrv_err("linenode:%p id:%c%c%c\n",lq,);
    ...
    *button =  _button;
    ...
    if(button->x > sdk_handle->scree_w){
        ...
        xw_logsrv_err("you set x > srcee_w id:%c%c%c\n",attr.node_id[0],attr.node_id[1],
                attr.node_id[2]);
    }
    if(button->y > sdk_handle->scree_h){
        button->y = sdk_handle->scree_h -button->h;
        xw_logsrv_err("you set y > srcee_w id:%c%c%c\n",attr.node_id[0],attr.node_id[1],
                attr.node_id[2]);
    }
...
int    Image_SDK_Create_Menu(struct user_set_node_atrr attr,
    ...
    *menu =  _menu;
    ...
    if(menu->w > sdk_handle->scree_w){
        menu->w = 60;
        xw_logsrv_err("you set w > srcee_w, w:%d h:%d  scee_w:%d sree_h:%d now set 60 pixl\n",
                menu->w,menu->h,sdk_handle->scree_w,sdk_handle->scree_h);
        ...
        window_node_text_t _text)
{
    ...
    uint8_t asc_width = sdk_handle->font_w;//image_text_get_font_width();
    uint8_t font_size = sdk_handle->font_h; //image_text_get_font_size();
...
int     Image_SDK_Set_Node_En(char *node_id,uint8_t en)
{
    ...
    do{
        if(en == 0)
            break;
        xw_logsrv_debug("maybe move this window :%s to free head\n",node_id);
...
{
	if(w < 0|| w > sdk_handle->scree_w || h < 0 || h > sdk_handle->scree_h )
		return -1;
	int level = 0;
    	level = node_id_level_re(node_id);
    	window_node_t *temp = NULL;
    	temp =  find_all_key_node(node_id,level);
	if(temp == NULL){
		xw_logsrv_err("not find node:%s\n",node_id);
		return -2;
	}
	
	window_node_menu_t *menu = (window_node_menu_t *)temp->window;
	menu->w = w;
	menu->h = h;
	return 0;
...
int     Image_SDK_Set_Bar_Vaule(char *node_id,uint16_t vaule)
{
    ...
    window_node_bar_t *bar  =(window_node_bar_t *)temp->window;
    bar->now_value = vaule;
    if(bar->now_value > bar->max_value)
        bar->now_value = bar->max_value;
    return 0;
...
static inline int image_bar_xy_analysis(void *data,
    ...
    if( (mdata.x)  >= (bt->x - 20)  && (mdata.x ) <= (bt->x + bt->w + 20)  
            && (mdata.y ) >= bt->y - 2  && (mdata.y) <= (bt->y+bt->h+2))
     {
        //add mouse data
    ...
    while ( node  ){
        ...
        if(ret > 0)
        {
            if( check_cnt == 0 || (check_cnt > 0 && node->f_node == save_node[check_cnt - 1])){
              ...
              //  xw_logsrv_err("check node_id:%c%c node:%p  node->shead:%p check_cnt:%d mouserx:%d mousry;%d \n\n",node->node_id[0],node->node_id[1],node->s_head,node,check_cnt,mdata.x,mdata.y);
            }
        }
...
static window_func_t inline *get_window_func(window_node_t *node)
{
    ...
    switch(node->win_type){
        ...
        case OBJECT_LINE:
          ...
          //  xw_logsrv_err("debug get line set:%p set->left_dow:%p lup:%p,data:%p\n",func,
            //        func->mouse_left_down,func->mouse_left_up,func->data);
            break;
        case OBJECT_TEXT_WIN:
...
static void  _image_window_func_run(void *data)
{
    ...
    for(k = MENU_LEVEL -1  ; k >= 0; k--){
        ...
        //close reflect
        sdk_handle->mouse->reflect_state = 0;
   ...
   //sdk_handle->check_level_cnt = 0; 
    ...
    // mouse rereflect build
    
    if(node->mouse_garp_reflect != 0)
    {
        switch(sdk_handle->mouse_new_data.event)
        {
            case GK_MOUSE_OFFSET:
                if(node->mouse_garp_reflect & OFFSET_REFLECT){
                    sdk_handle->mouse->reflect_state = 1;
                }
                break;
            case GK_MOUSE_LEFT_UP:
                if(node->mouse_garp_reflect & LUP_REFLECT){
                    sdk_handle->mouse->reflect_state = 4;
                }

                break;
            case GK_MOUSE_LEFT_DOWN:
                if(node->mouse_garp_reflect & LDOWN_REFLECT){
                    sdk_handle->mouse->reflect_state = 2;
                }
...
static void freshen_image_buttion(void *data){
    ...
    if(bt->user_video_freshen){
        bt->user_video_freshen(data,buf,sdk_handle->base_scree_w,sdk_handle->base_scree_h);
    ...
    if(bt->this_node->freshen_arrt  == NEED_CLEAR){
        ...
        for(k = bt->last_y ; k < (bt->h + bt->last_y) ;k++){
            for(i = bt->last_x ;  i < (bt->w + bt->last_x) ; i++ )
                *(buf+ sdk_handle->base_scree_w*k + i) = 0;    
        }
        ...
        for(s = 0 ,k = bt->y ; k < (bt->h + bt->y) ;k++){
            for(i = bt->x ;  i < (bt->w + bt->x) ; i++,s++ )
                if(bt->image_cache){
                    *(buf+ sdk_handle->base_scree_w*k + i) = *(((int16_t *)bt->image_cache) +s);    
                }else{
                    *(buf+ sdk_handle->base_scree_w*k + i) = bt->color;
                }
        }
        bt->last_x = bt->x;
        bt->last_y = bt->y;
        ...
        if(bt->this_node->move_arrt != NOT_MOVE){
            for(k = bt->last_y ; k < (ho + bt->last_y) ;k++){
                for(i = bt->last_x ;  i < (bt->last_x + wo)  ; i++ )
                {
                    ...
                    *(buf+ sdk_handle->base_scree_w*k + i) = 0x0;    
                }
            }
        }
    ...
    if(bt->this_node->freshen_arrt  == NEED_CLEAR){
        ...
        for(k = bt->y ; k < (bt->h + bt->y) ;k++){
            for(i = bt->x ;  i < (bt->x +bt->w) ; i++ )
                *(buf+ sdk_handle->base_scree_w*k + i) = 0xea10;    
        }
        ...
        for(k = bt->y ; k < (bt->h + bt->y) ;k++){
            for(i = bt->x ;  i < ( bt->x + vl )  ; i++ )
                *(buf+ sdk_handle->base_scree_w*k + i) = bt->bar_color;    
        }
        bt->last_x = bt->x;
        bt->last_y = bt->y;
        ...
        if(bt->nohavetext == 0)
        {
            sprintf(xbuf,"%d",bt->now_value);
            Image_SDK_Set_Text_Node_Text(bt->text_id, xbuf,strlen(xbuf));
            Image_SDK_Set_Node_En_Freshen(bt->text_id,NEED_FRESHEN);
        }
    }
...
static void freshen_image_text(void *data){
    ...
    if(bt->this_node->freshen_arrt  == NEED_CLEAR){
        ...
        for(k = bt->last_y ; k < (bt->font_size + bt->last_y) ;k++){
            ...
            {
                if(bt->this_node->move_arrt == NOT_MOVE)
                    *(buf+ sdk_handle->base_scree_w*k + i) =  0x0;//bt->win_color;  
                else
                    *(buf+ sdk_handle->base_scree_w*k + i) = 0x0;  
        ...
        if(bt->last_x != 0 && bt->last_y != 0){
            for(k = bt->last_y ; k < (bt->font_size + bt->last_y) ;k++){
                ...
                for(i = bt->last_x ;  i < (bt->asc_width *bt->lens + bt->last_x) ; i++ ){
                    ...
                    if(bt->this_node->move_arrt == NOT_MOVE){
                        *(buf+ sdk_handle->base_scree_w*k + i) = bt->win_color;
        ...
        text_bit = (uint8_t *)buddymem_alloc(sdk_handle->mems, bt->asc_width * bt->font_size);
        if(!text_bit ){
            xw_logsrv_err("malloc mem for text bit buf fail node id:%c%c%c w:%d h:%d \n\n",bt->this_node->node_id[0],
                    bt->this_node->node_id[1],bt->this_node->node_id[2],bt->asc_width,bt->font_size);
            return;
        } 
        ...
        for(nums = 0; nums < bt->lens; nums++){
            ...
            ret = image_text_lib_put_pixl_v2(&s,text_bit,bt->asc_width,bt->font_size);
            if(ret < 0)
                break;
            ...
            for(k = bt->y ; k < (bt->font_size + bt->y) ;k++){
                for(i = bt->x + nums*bt->asc_width ;  i < (bt->x + nums*bt->asc_width+bt->asc_width) ; i++,j++ )
                {
                    if(text_bit[j] == 0)
                        ...
                        *(buf+ sdk_handle->base_scree_w*k + i) = bt->text_color;  
                }
            }
        }
    ...
    buddymem_free(sdk_handle->mems, text_bit);
...
#define  MOUSE_LINE_SIZE    2
    ...
    uint16_t *savebuf ,*srcbuf,set_w,set_h;
    ...
    if(bt->mouse_buf)
    {
        ...
        if(bt->last_reflect_state != CLOSE_REFLECT){
            savebuf = (uint16_t *)bt->save_cache_v2;
            srcbuf  = bt->mouseh_buf;
            set_w = bt->sizew_v2;
            set_h = bt->size_v2;
        }else{
        ...
        //reset data
        for(k = bt->y_last ; k < (set_h + bt->y_last); k++)
        {
            for(i = bt->x_last ; i < (set_w + bt->x_last) ;i++)
            {

                if( *(buf+ sdk_handle->base_scree_w*k + i) == *srcbuf  && ((*srcbuf&0xf000) != 0) )
                {  
                    *(buf+ sdk_handle->base_scree_w*k + i) = *savebuf++;
                }else
                {
                    savebuf++; 
        ...
        //save data
        if(bt->reflect_state != CLOSE_REFLECT){
            savebuf = (uint16_t *)bt->save_cache_v2;
            srcbuf  = bt->mouseh_buf;
            set_w = bt->sizew_v2;
            set_h = bt->size_v2;
        ...
        for(k = bt->y ; k < (set_h + bt->y); k++)
        {
            for(i = bt->x ; i < (set_w + bt->x) ;i++)
            {
                *savebuf++ = *(buf+ sdk_handle->base_scree_w*k + i);
            }
        }
        ...
        for(k = bt->y ; k < (set_h + bt->y); k++)
        {
            for(i = bt->x ; i < (set_w + bt->x) ;i++)
            {
                ...
                if( (*srcbuf&0x8000) != 0x0)
                {  
                    *(buf+ sdk_handle->base_scree_w*k + i) = *srcbuf;
                }
                srcbuf++;
            }
        }
        
        bt->last_reflect_state = bt->reflect_state;
        ...
        for(k = bt->y_last + bt->size/2; k < (MOUSE_LINE_SIZE + bt->y_last+bt->size/2) ; k++)
        {
            for(i = bt->x_last ;  i < (bt->size + bt->x_last) ; i++){
                if( *(buf+ sdk_handle->base_scree_w*k + i) == bt->color )  //if mouse use image ,this need change
                    *(buf+ sdk_handle->base_scree_w*k + i) = *savebuf++;
                else
                {
                    savebuf++; 
        ...
        for(k = bt->x_last + bt->size/2 ; k < (MOUSE_LINE_SIZE + bt->x_last+bt->size/2) ; k++)
        {
            for(i = bt->y_last ;  i < (bt->size + bt->y_last) ; i++){
                if(*(buf+ sdk_handle->base_scree_w*i + k) == bt->color) //if mouse use image ,this need change        
                    *(buf+ sdk_handle->base_scree_w*i + k) = *savebuf++;
                else{
                    savebuf++;
                }
            }
        ...
        for(k = bt->x + bt->size/2 ; k < (MOUSE_LINE_SIZE + bt->x + bt->size/2) ; k++)
        {
            for(i = bt->y ;  i < (bt->size + bt->y) ; i++ )
                *savebuf++  = *(buf+ sdk_handle->base_scree_w*i + k);
        } 
        ...
        for(k = bt->y + bt->size/2 ; k < (MOUSE_LINE_SIZE + bt->y+bt->size/2) ; k++)
        {
            for(i = bt->x ;  i < (bt->size + bt->x) ; i++ ){
                *(buf+ sdk_handle->base_scree_w*k + i) = bt->color;
                // xw_logsrv_debug("mem :%d \n",*(buf + sdk_handle->base_scree_w*k +i));
            }
        }
        for(k = bt->x + bt->size/2 ; k < (MOUSE_LINE_SIZE + bt->x + bt->size/2) ; k++)
        {
            for(i = bt->y ;  i < (bt->size + bt->y) ; i++ )
                *(buf+ sdk_handle->base_scree_w*i + k) = bt->color;
        }
    } 
...
{
    //init host handle
    image_host_handle_t     host_ht;
    host_ht.retype    = IMAGE_HOST_NOTHING;
    host_ht.host_fd   = -1;

    host_ht.host_fd = image_host_event_dect_init();
    if(host_ht.host_fd < 0){
    
        xw_logsrv_err("init host event handle fail\n");
    }
    //mouse data create
    GK_MOUSE_DATA       mdata;
    memset(&mdata,0x0,sizeof(GK_MOUSE_DATA));
    
    //about select handle data cerate
    fd_set              readfds;
    struct  timeval     tv;
    tv.tv_sec = 1;
    tv.tv_usec = 0 ;
    int                 retval = 0,setcnt = 0,max_fd = 0;

    //set the thread work state
    ...
    while(1)
    {
        ...
        //
        if(host_ht.retype == IMAGE_MOUSE_HOST_MOVE ){
             
            close(sdk_handle->mouse_fd);
            sdk_handle->mouse_fd = -1;
            host_ht.retype = IMAGE_HOST_NOTHING;
            xw_logsrv_err("move mouse \n");   
        }
        if(host_ht.retype ==IMAGE_MOUSE_HOST_INSTER){
            usleep(1000* 3000);
            sdk_handle->mouse_fd =  
                gk_mouse_open(GK_DEVICE_MOUSE,sdk_handle->scree_w,sdk_handle->scree_h,GK_MOUSE_SPEED,MOUSE_SIZE);
            host_ht.retype = IMAGE_HOST_NOTHING;
            xw_logsrv_err("add mouse \n");
        }
        ...
        //add host fd
        if(host_ht.host_fd > 0){
            FD_SET(host_ht.host_fd,&readfds);
            setcnt++;
            max_fd = host_ht.host_fd;
        }
        ...
        if(sdk_handle->mouse_fd > 0){
         //   FD_ZERO(&readfds);
            ...
            setcnt++;
            if(sdk_handle->mouse_fd > max_fd)
                max_fd = sdk_handle->mouse_fd;
        }
        ...
        if(retval <= 0)
        {
          //  xw_logsrv_err("select not event or err reval:%d mosue_fd:%d host_fd:%d\n",retval,
                 //   sdk_handle->mouse_fd,host_ht.host_fd);
            continue;
        }
        if(sdk_handle->mouse_fd > 0){
        if(FD_ISSET(sdk_handle->mouse_fd,&readfds))
            {
                if(gk_mouse_read_data(sdk_handle->mouse_fd,&mdata) >= 0 )
                {
                   ...
                   // xw_logsrv_err("get mouse data x:%d y:%d \n",mdata.x,mdata.y);
                    sdk_handle->mouse_data_updated = 1;
                }
            }
        }
        if(FD_ISSET(host_ht.host_fd,&readfds)){
            image_host_event_dect_process(&host_ht);
        }

    }
    
    
    image_host_event_dect_deinit(host_ht.host_fd);
...
static void* _image_sdk_handle_data_process_thread(void *data)
{
    ...
    while(1)
    {
        ...
        if(sdk_handle->mouse_data_updated){
            ...
            //updating not run other func
            if(sdk_handle->en != SDK_WORK_STATE_UPDATE)
            {
                _image_analysis_mdata(sdk_handle->mouse_new_data);
                _image_window_func_run(NULL);
                //xw_logsrv_err("need analysis mouse data \n");
            }
...
static uint16_t  argb1555to_ayuv(uint16_t argb){

            
            uint8_t a,b,g,r,y,u,v;
            uint16_t ayuv;
            ...
            ayuv  = ((a << 15) | (y << 10) | (u << 5) | (v << 0));
            

            xw_logsrv_err("argb:%x ayuv:%x,a:%d b:%d g:%d r:%d\n",argb,ayuv,a,b,g,r);
            return ayuv;
...
static void*  _image_fb_init(char *dev_path,int *fd)
{
    ...
    xw_logsrv_err("fscreeinfo fixInfo: xpan:%d ypan:%d smem_len:%d \n",fixInfo.xpanstep,fixInfo.ypanstep,fixInfo.smem_len);
    ...
    xw_logsrv_err("varInfo: varinfo.xre:%d yre:%d xres_virtual:%d yres_virtual:%d w:%d h:%d \n",
            varInfo.xres,varInfo.yres,varInfo.xres_virtual,varInfo.yres_virtual,varInfo.width,varInfo.height);

    varInfo.xres            =  sdk_handle->scree_w;
    varInfo.yres            =  sdk_handle->scree_h;
    varInfo.xres_virtual    =  0;//sdk_handle->scree_w;
    varInfo.yres_virtual    =  0;//sdk_handle->scree_h;
    varInfo.bits_per_pixel  =  16; //VO_COLOR_FMT;
    varInfo.width           = sdk_handle->scree_w;
    varInfo.height          = sdk_handle->scree_h;
    //set 
    ...
    xw_logsrv_err("framebuffer: %d x %d @ %d -- %dbpp\n", varInfo.xres, varInfo.yres,
    ...
    xw_logsrv_err("framebuffer addr:0x%08x, len=%d\n", (int)fbBuffer, fixInfo.smem_len);
    ...
    uint16_t   bule ,red,green,wt ;
    
    bule = argb1555to_ayuv(0xf7db);

    red  = argb1555to_ayuv(0x801f);
    
    green = argb1555to_ayuv(0xfc00);

    wt  = argb1555to_ayuv(0xffff);

       

    for(y = 0 ;y < varInfo.yres/2;y++){
        for(x = 0 ;x < varInfo.xres/2; x++){
            *(baseAddr + x + y*(fixInfo.line_length/4)) = bule; //0x83e0; 
        }
        for(; x < varInfo.xres; x++){
            
             *(baseAddr + x +y*(fixInfo.line_length/4)) =    0x0;//green;//argb1555to_ayuv(0xfc00);  //0xfc00; 
    ...
    for(; y < varInfo.yres;y++){
        for(x = 0; x < varInfo.xres/2;x++){
            *(baseAddr + x +y*(fixInfo.line_length/4)) =     red;//argb1555to_ayuv(0x801f); //0x801f; 
        }
        for(; x < varInfo.xres;x++){
            *(baseAddr + x + y*(fixInfo.line_length/4)) =     wt;//argb1555to_ayuv(0xffff);//0xffff;
        }
...

{
    ...
    munmap(mmap_p, sdk_handle->scree_h * sdk_handle->scree_w *(VO_COLOR_FMT/8));
    close(fd);
    return NULL;
}
...
{
    if(sdk_handle)
        sdk_handle->en = SDK_WORK_STATE_UPDATE;
    return;
...
{
    if(sdk_handle)
        sdk_handle->en = SDK_WORK_STATE_QUIT;
    return;
